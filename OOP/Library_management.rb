...
if (File.file?("./Books.json") && !File.zero?("./Books.json"))
    $user_array = JSON.parse(File.read("./Books.json"))
else
...
def book_searching
    ...
    $books_array.each_with_index { |value, index|
...
#Sua thong tin sach co san (dung chuc nang tim kiem de chon duoc ban ghi muon chinh sua)
def book_editing
    book_searching
